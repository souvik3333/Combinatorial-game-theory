...
int minimax(int a[], node b[],int n,int e, bool isMax,int i)
{
    if(b[i].value != -1)
        return b[i].value;
    if(isMax){
    	int ans=0;
    	for(int j=0;j<a[i];j++){
    		ans=max(ans,minimax(a,b,n,e,0,b[i].next[j]));
    	}
       	return ans;
    }
    else{
        int ans=99999;
    	for(int j=0;j<a[i];j++){
    		ans=min(ans,minimax(a,b,n,e,1,b[i].next[j]));
    	}
       	return ans;
    }   	
}
